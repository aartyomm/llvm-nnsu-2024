...
bool X86KostinPass::runOnMachineFunction(MachineFunction &pFunction) {
  ...
  for (auto &basicBlock : pFunction) {
    ...
    for (auto &instr : basicBlock) {
      if (instr.getOpcode() == X86::MULPDrr) {
        ...
        for (auto next = std::next(instr.getIterator()); next != basicBlock.end(); ++next) {
          if (next->getOpcode() == X86::ADDPDrr) {
            addInstruction = &*next;
            if (mulInstruction->getOperand(0).getReg() == addInstruction->getOperand(1).getReg()) {
              toDelete.emplace_back(mulInstruction, addInstruction);
              isModified = true;
              break;
            }
          } else if (next->definesRegister(mulInstruction->getOperand(0).getReg())) {
            break;
          }
        }
      }
  ...
  for (const auto &[mulInstr, addInstr] : toDelete) {
    MachineInstrBuilder builder = BuildMI(*mulInstr->getParent(), *mulInstr, mulInstr->getDebugLoc(),
