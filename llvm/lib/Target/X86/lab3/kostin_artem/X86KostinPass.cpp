...
#define NAME "x86-kostin-pass"
#define DESC "pass replacing mul and add with muladd"

#define DEBUG_TYPE AVOIDCALL_NAME

namespace {
class KostinPass : public MachineFunctionPass {
public:
  static char ID;
  KostinPass() : MachineFunctionPass(ID) {}
...
char KostinPass::ID = 0;

FunctionPass *llvm::createX86KostinPass() {
  return new KostinPass();
}

INITIALIZE_PASS(KostinPass, NAME, DESC, false, false)
...
bool KostinPass::runOnMachineFunction(MachineFunction &MF) {
